...
from lunamoth.obs import broker, get_logger, setup_logging


def test_files_redaction_and_session_tag(tmp_path, monkeypatch):
    monkeypatch.setenv("LUNAMOTH_SESSION", "testchara")
    ...
    main = (tmp_path / "lunamoth.log").read_text(encoding="utf-8")
    errors = (tmp_path / "errors.log").read_text(encoding="utf-8")
    assert "[testchara]" in main and "lunamoth.unittest" in main
...
def test_debug_level_toggle(tmp_path):
    ...
    assert "debug detail visible" in (tmp_path / "lunamoth.log").read_text(encoding="utf-8")
