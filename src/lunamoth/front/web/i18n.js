...
const I18N = {
  ...
  "life-next-min": ["约 {n} 分钟后开始自己的事", "Starts its own work in ~{n} min"],
  ...
  "p-menu-tempo": ["节奏与陪伴", "Tempo & presence"],
  ...
  "p-tempo": ["时间流速", "Time flow"],
  "p-tempo-sub": ["只缩放自发周期的间隔，不动等待与休息", "Scales only the spontaneous pause, never quiet or rest"],
  ...
  "works-preview-wait": ["应用内预览等待后端（works.read）——点击用系统打开。", "In-app preview awaits the backend (works.read) — click opens with the system."],
  "open-sandbox": ["打开沙盒文件夹", "Open sandbox folder"],
  // terminal page
  "term-wait": ["操作员终端（PTY）后端尚未就绪。这是 TA 沙盒的地址：", "The operator terminal (PTY) backend is not ready yet. This is its sandbox:"],
  "term-note": ["就绪后，这里会是一个直通 TA 沙盒的 shell（TUI 的 `!`）。", "Once ready, this becomes a shell straight into its sandbox (the TUI's `!`)."],
  "copy": ["复制", "Copy"],
  ...
  "av-ai-wait": ["等待后端（card.avatar_draft）——现在可以改主题色，或直接编辑 SVG。", "Awaits the backend (card.avatar_draft) — for now, change the theme color or edit the SVG directly."],
  ...
  // gateway modal
  "gw-title": ["{name} 的网关", "{name}'s gateway"],
  ...
  "gw-wait-backend": ["配置读写等待后端（messaging.get / messaging.save）——以下为只读预览。", "Config read/write awaits the backend (messaging.get / messaging.save) — read-only preview below."],
  ...
  "gw-weixin-qr": ["扫码流程等待后端（weixin.qr）——现在：启动网关后在终端扫码登录。", "The QR flow awaits the backend (weixin.qr) — for now: start the gateway, then scan in the terminal."],
