...
def language() -> str:
    lang = os.getenv("LUNAMOTH_LANG", os.getenv("LUNAMOSS_LANG", "zh")).strip().lower()
...
def default_character_path() -> Path | None:
    """Bundled default character (LunaMoth 月蛾) for the active language, if present."""
    p = ROOT / "characters" / f"LunaMoth.{language()}.json"
    return p if p.exists() else None


def default_world_path() -> Path | None:
    """World book that pairs with the default character, if present."""
    p = ROOT / "worlds" / f"LunaMoth.{language()}.json"
    return p if p.exists() else None
