...
- **随附的 SCP 衍生示例内容**（`cards/` 下的 SCP-079 角色卡，含其内嵌世界书）：[CC BY-SA 3.0](CONTENT_LICENSE.md)，与 SCP Wiki 一致。另见 [NOTICE.md](NOTICE.md)。LunaMoth 原创资产（月蛾与 Quinn 小Q 的卡及其内嵌世界书）与项目主体一致，采用 Apache-2.0。

这个项目的起点是一个 SCP 同人作品：尝试在现实世界中复现 **SCP-079** —— 一个资源受控、永远清醒、永远憎恨的旧 AI。它很快被扩展为通用的 roleplay agent 系统。LunaMoth 月蛾是 079 的反面：同样受制于茧房之中，却高尚而乐于助人——它作为旗舰示例卡随仓库附带（默认角色是数字实习生 Quinn 小Q）；而使用 079 应被视为同人创作，不包含真实的恶性意图。感谢 SCP Wiki 上 SCP-079 的原作者，也感谢 SillyTavern 社区中 SCP-079 角色卡与 SCP 基金会世界书的作者——这些素材作为示例内容随本仓库分发（世界书现已内嵌在 079 的卡里）。移除或替换这些资产后，运行时仍是纯 Apache-2.0 代码；若再分发它们，请遵守 CC BY-SA 的署名与相同方式共享条款。
