...
- [x] **Messaging 网关（企业微信 + 个人微信 + QQ + Telegram）** —— `lunamoth gateway NAME` 在一个已激活 chara 后运行 `~/.lunamoth/sessions/NAME/messaging.json`；适配器只投递 `say` 文本（包括 idle 中 `speak` 工具说给用户的话），muse / thinking / tool 事件都不出门。企业微信自建应用使用 stdlib 回调服务器；回调加解密用可选依赖 `uv sync --extra messaging`。
