...
- [x] 兼容 SillyTavern 的角色卡；独立世界书以"并入卡内嵌 `character_book`"的方式导入
...
- [x] **Messaging 网关（企业微信 + 个人微信 + QQ + Telegram）** —— `lunamoth gateway NAME` 在一个已激活 chara 后运行 `~/.lunamoth/sessions/NAME/messaging.json`；适配器只投递 `say` 文本（包括 idle 中 `speak` 工具说给用户的话），muse / thinking / tool 事件都不出门。企业微信自建应用使用 stdlib 回调服务器；回调加解密用可选依赖 `uv sync --extra messaging`。个人微信有两条路：官方 iLink/ClawBot（`weixin`，封号风险最低但有灰度门槛），或者通过自建的 WeChatPadPro docker 容器（`weixinpad`，iPad 协议，生成可在任意账号扫描登录的真·设备登录二维码）。
  ...
  **个人微信 / WeChatPadPro 设置（`weixinpad`）：** 如果你的账号还没拿到 ClawBot 灰度（iLink 二维码“扫了啥都不是”），可以自己跑一套 [WeChatPadPro](https://github.com/WeChatPadPro/WeChatPadPro) docker（容器 + MySQL + Redis；默认 API 端口 38849，自己设一个 `adminKey`），把本适配器指过去。首次运行时网关会用你的 `adminKey` 派生出一个 per-account auth key，打印一个**真正的微信设备登录二维码**（用手机微信扫一次 → “在其它设备登录”）；之后通过 `GetSyncMsg` WebSocket 收消息、通过 HTTP REST 发消息。登录凭据保存在会话目录的 `weixinpad_state.json`，不写进 `messaging.json`。WebSocket 传输需要 `websockets`（`uv sync --extra server`）。v1 只支持文本。和 iLink 不同，它通常可以主动给任意好友发消息，所以一旦有了目标对象，unattended `speak` 就能用。**封号风险是真实存在的：** WeChatPadPro 走的是微信非官方 iPad 协议——Tencent 不认可。上游警告：新账号上高风险操作前最好先“稳定 3 天”，新登录可能在 24 小时内被强制下线，加好友 / 批量操作会触发 7 到 30 天封禁。请用小号 / 备用微信，手机保持同区域登录，并把发送频率压低。

  ```json
  {
    "allowed_senders": ["<第一次接触后看到的对方 wxid>"],
    "adapters": {
      "weixinpad": {
        "host": "127.0.0.1",
        "port": 38849,
        "admin_key": "<你的 WeChatPadPro adminKey>"
      }
    }
  }
  ```
  ...
  **Telegram 设置：** 在 `@BotFather` 创建 bot，然后在 `adapters` 下加入 `"telegram": {"bot_token": "<token>"}`（可选 `api_base`，用于自建 Bot API 服务器）；网关用 `getUpdates` 长轮询——不需要公网 URL 或 webhook——offset 持久化在会话目录的 `telegram_state.json`，重启不会重放旧消息；v1 只处理私聊文本。Telegram bot 不能主动联系从未私聊过它的用户，所以首次接触前的 unattended `speak` 会清楚记录为 deferred，不走任何 fallback。
